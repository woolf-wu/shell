...
## status
phenoData$status <- ifelse(phenoData$vital_status.diagnoses == "alive", 0, 1)
table(phenoData$status)

## type
phenoData$histological_type <- phenoData$histological_type
table(phenoData$histological_type)

library("stringr")
## stage
phenoData$tumor_stage <- str_split(phenoData$tumor_stage.diagnoses, ' ', simplify = T)[, 2]
phenoData$tumor_stage <- ifelse(phenoData$tumor_stage %in% c("i", "ia", "ib", "ii", "iia", "iib"),
                                "early_stage", "later_stage")
table(phenoData$tumor_stage)

## smoke
phenoData$smoked <- ifelse(is.na(phenoData$number_pack_years_smoked), 0, phenoData$number_pack_years_smoked)
phenoData$smoked_type <- ifelse(phenoData$smoked < median(phenoData$smoked), "Shorter_smoking_time", "Longer_smoking_time")
...
## cigarettes
phenoData$cigarettes_per_day.exposures <- ifelse(is.na(phenoData$cigarettes_per_day.exposures), 0, phenoData$cigarettes_per_day.exposures)
phenoData$cigarettes_type <- ifelse(phenoData$cigarettes_per_day.exposures < median(phenoData$cigarettes_per_day.exposures), "Shorter_exposure_time", "Longer_exposure_time")
table(phenoData$cigarettes_type)
...
## 是否要挑选亚型进行研究
sub_sample <- rownames(phenoData)[phenoData$histological_type == names(table(phenoData$histological_type))[2]] 
...
survival_dat <- data.frame(gender = pheno$gender,
                           ...
                           cigarettes_type = pheno$cigarettes_type,
                           stringsAsFactors = F)
...
pick_gene <- intersect(single_pick, choose_gene)
...
multi_results <- apply(t_exp[pick_gene, ] , 1 , function(gene){
  ...
  survival_dat <- data.frame(group = group,
                             ...
                             cigarettes_type = pheno$cigarettes_type,
                             stringsAsFactors = F)
  res.cox <- coxph(Surv(time, status) ~ smoked_type + gender + cigarettes_type + group, 
                   data =  survival_dat)
  ## summary(res.cox)
  x <- summary(res.cox)
...
arrange_ggsurvplots(os.paint, print = TRUE, ncol = 3, nrow = 2)
...
library("survminer")
colnames(dat)
s <- Surv(time, status) ~ BANCR + LINC00211 + LINC01625 + LINC01169 + FENDRR
...
plot(ROC, time = 120, col = "blue", add = T)

ROC$AUC

legend("top",
       legend = c("t=120, AUC=59.8","t=60, AUC=62.3"),
       ncol = 4,
       cex = 0.8,
       bty = "n",
       col = c("blue","red"),
...
e_phe <- pheno[pheno$smoked_type == "Shorter_smoking_time", ]
l_phe <- pheno[pheno$smoked_type == "Longer_smoking_time", ]
